...
import { getOrSetCachedPromise } from '@/app/lib/requestCache';
...
export async function GET(req: Request) {
  try {
    ...
    if (configured) {
      ...
      profile = await getOrSetCachedPromise(`user-profile-role-${userId}`, async () => {
      ...
      allProjs = await getOrSetCachedPromise('dashboard-all-active-projects', async () => {
        const { data } = await supabaseAdmin.from('projects').select('*').ilike('status', 'active');
        return data || [];
      }, 10000);

      allHolders = await getOrSetCachedPromise('dashboard-all-rights-holders', async () => {
        const { data } = await supabaseAdmin.from('rights_holders').select('*');
        return data || [];
      }, 10000);

      // Filter transactions by is_demo
      const txCacheKey = `dashboard-txs-demo-${isDemoMode}`;
      allTx = await getOrSetCachedPromise(txCacheKey, async () => {
